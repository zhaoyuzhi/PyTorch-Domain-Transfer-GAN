...
# the output channel is not as same as the input channel
class BicycleGANResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, pad_type = 'zero', activation = 'lrelu', norm = 'in', sn = False):
        super(BicycleGANResBlock, self).__init__()
        self.conv = nn.Sequential(
            nn.InstanceNorm2d(in_channels),
            nn.LeakyReLU(inplace = True),
            Conv2dLayer(in_channels, in_channels, 3, 1, 1, 1, pad_type, activation = activation, norm = norm, sn = sn),
            Conv2dLayer(in_channels, out_channels, 4, 2, 1, 1, pad_type, activation = 'none', norm = 'none', sn = sn)
        )
        self.short_cut = Conv2dLayer(in_channels, out_channels, 4, 2, 1, 1, pad_type, activation = 'none', norm = 'none', sn = sn)
